"use client";

import { useEffect, useState, useCallback } from "react";
import {
  LineChart,
  Line,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  ResponsiveContainer,
} from "recharts";

interface ChartPoint {
  round: string;
  pnl: number;
  totalValue: number;
}

interface Props {
  eventId: string;
  currentRound: number;
}

// Custom tooltip styled to match the design system
function CyberpunkTooltip({
  active,
  payload,
  label,
}: {
  active?: boolean;
  payload?: { name: string; value: number; color: string; dataKey: string }[];
  label?: string;
}) {
  if (!active || !payload?.length) return null;
  const pnlRow = payload.find((p) => p.dataKey === "pnl");
  const valueRow = payload.find((p) => p.dataKey === "totalValue");
  const pnl = pnlRow?.value ?? 0;
  const totalValue = valueRow?.value ?? 0;
  return (
    <div className="bg-black border border-green-500/30 rounded px-3 py-2 font-mono text-xs space-y-1">
      <p className="text-green-700 tracking-widest uppercase">Round {label}</p>
      <p
        className={
          "tabular-nums " + (pnl >= 0 ? "text-green-400" : "text-red-400")
        }
      >
        P&amp;L: {pnl >= 0 ? "+" : ""}₨
        {pnl.toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}
      </p>
      <p className="text-green-300 tabular-nums">
        Value: ₨
        {totalValue.toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}
      </p>
    </div>
  );
}

export default function StockChart({ eventId, currentRound }: Props) {
  const [chartData, setChartData] = useState<ChartPoint[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const fetchChart = useCallback(async () => {
    try {
      const res = await fetch(`/api/game/charts?eventId=${eventId}`);
      if (!res.ok) return;
      const data = (await res.json()) as { series: PnlPoint[] };

      if (!data.series.length) {
        setIsLoading(false);
        return;
      }

      const sorted = data.series
        .map((point) => ({
          round: `R${point.round}`,
          pnl: point.pnl,
          totalValue: point.totalValue,
        }))
        .sort(
          (a, b) =>
            parseInt(String(a.round).slice(1)) -
            parseInt(String(b.round).slice(1)),
        );

      setChartData(sorted);
    } catch {
      /* silent */
    } finally {
      setIsLoading(false);
    }
  }, [eventId]);

  useEffect(() => {
    void fetchChart();
  }, [fetchChart, currentRound]); // refetch when round changes

  if (isLoading)
    return (
      <div className="bg-[#0a0a0a] border border-green-500/20 rounded-md p-4">
        <p className="text-xs text-green-900 tracking-widest text-center py-8 animate-pulse">
          LOADING CHART DATA...
        </p>
      </div>
    );

  if (!chartData.length) return null;

  return (
    <div className="bg-[#0a0a0a] border border-green-500/20 rounded-md p-4 space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs uppercase tracking-widest text-green-700">
          My P&amp;L History
        </p>
        <p className="text-xs text-green-900 tracking-widest">
          ROUND {currentRound}
        </p>
      </div>

      <ResponsiveContainer width="100%" height={240}>
        <LineChart
          data={chartData}
          margin={{ top: 4, right: 8, left: 0, bottom: 0 }}
        >
          <CartesianGrid strokeDasharray="3 3" stroke="#14532d30" />
          <XAxis
            dataKey="round"
            tick={{ fill: "#166534", fontSize: 10, fontFamily: "monospace" }}
            axisLine={{ stroke: "#14532d50" }}
            tickLine={false}
          />
          <YAxis
            tick={{ fill: "#166534", fontSize: 10, fontFamily: "monospace" }}
            axisLine={{ stroke: "#14532d50" }}
            tickLine={false}
            tickFormatter={(v: number) => `${v >= 0 ? "+" : ""}₨${v}`}
            width={60}
          />
          <Tooltip content={<CyberpunkTooltip />} />
          <Line
            type="monotone"
            dataKey="pnl"
            name="P&L"
            stroke="#22c55e"
            strokeWidth={2}
            dot={{ fill: "#22c55e", r: 3, strokeWidth: 0 }}
            activeDot={{ r: 5, strokeWidth: 0 }}
          />
          <Line type="monotone" dataKey="totalValue" stroke="transparent" dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
}

interface PnlPoint {
  round: number;
  pnl: number;
  totalValue: number;
}
