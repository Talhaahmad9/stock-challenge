...
export async function executeTrade(params: {
  ...
  try {
    ...
    // d. Get stock price and symbol for this round
    ...
    if (priceError || !priceData) {
      return {
        success: false,
        error: "Stock price not available for this round",
      };
    }

    const stockPrice = (priceData as unknown as StockPriceRow).price;

    // d2. Get stock symbol for socket broadcast
    ...
    const stockSymbol =
      (stockData as unknown as { symbol: string } | null)?.symbol ?? "";
    ...
    if (type === "BUY") {
      ...
      if (existingHolding) {
        ...
        const newAvg =
          (existing.quantity * existing.avg_buy_price + quantity * stockPrice) /
          newQty;
    ...
    const newHoldings = await fetchUpdatedHoldings(
      supabase,
      portfolio.id,
      roundId,
    );
...
export async function getPortfolio(
  ...
  try {
    ...
    const holdings = await fetchUpdatedHoldings(
      supabase,
      portfolio.id,
      roundId,
    );
