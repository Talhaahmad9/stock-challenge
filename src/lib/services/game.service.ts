...
async function applyTransition(eventId: string, newStatus: EventStatus): Result {
  const supabase = await db()
...
export async function initGameState(
  ...
  if (existing) {
    return { success: false, error: 'Game state already exists for this event' }
  }
...
export async function pauseGame(eventId: string): Result {
  ...
  if (currentStatus !== 'ROUND_ACTIVE' && currentStatus !== 'RUNNING') {
    return { success: false, error: `Cannot pause from state: ${currentStatus}` }
  }
...
export async function resetGame(eventId: string): Result {
  // Step 1: GAME_END → RESET
  const toReset = await transitionState(eventId, 'RESET')
  if (!toReset.success) return toReset
  ...
  const { error: eventError } = await supabase
    .from('events')
    .update({ status: 'IDLE', current_round: 0 })
    .eq('id', eventId)
...
export async function calculateFinalScores(
  ...
  const { data: holdings } = await supabase
    ...
    .in('portfolio_id', portfolioRows.map((p) => p.id))
